% [y,feature_names,cache] = ComputeMinWindowFeatures(x,...)
% 
% Computes the minimum window features y for the input one-dimensional
% per-frame time series data x. y(i,t) will correspond to minimum
% window feature i and frame t, and is the min of (the transformation of)
% the per-frame data within the window defined by i at t. 
% 
...
% feature y(i,t) is the min of the per-frame data within the window from
% t-r_i+off_i through t+r_i+off_i. If the transformation for feature i is
% 'abs', then the window feature y(i,t) is the min of the absolute value
% of the per-frame data within this same window. If the transformation for
% feature i is 'flip', then the window feature y(i,t) is the min of the
...
% a value, e.g. 
% {'stat','min','trans','abs','radius',1,'offset',1}
...
% computation. 
function [y,feature_names,cache] = ComputeMinWindowFeatures(x,varargin)
...
for radiusi = 1:nradii,
  ...
  % no need to do minimum for r == 0 if we've done mean or max
