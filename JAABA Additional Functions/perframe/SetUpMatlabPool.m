function nthreads = SetUpMatlabPool()
...
if matlabpool_size > 0,
  rc.computation_threads = min(matlabpool_size,NumWorkers-1);
end
...
if matlabpool_size>0 && matlabpool_size ~= rc.computation_threads,
  matlabpool close;
  matlabpool_size = matlabpool('size');
end
...
if (numCores>1) && (matlabpool_size<1),
  matlabpool('open',rc.computation_threads);
end
