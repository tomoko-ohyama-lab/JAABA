function d = ComputeDistFlyEll2Nose(obj,n,fly1)
...
for fly2 = 1:nflies,
  ...
  % position of nose
  xnose = x_mm2 + 2*a_mm2.*cos(theta_mm2);
  ynose = y_mm2 + 2*a_mm2.*sin(theta_mm2);
  ...
  for t = t0:t1,
    ...
    d(fly2,i) = ellipsedist_hack(x_mm1(i),y_mm1(i),...
      a_mm1(i),b_mm1(i),theta_mm1(i),...
      xnose(j),ynose(j));
  end
