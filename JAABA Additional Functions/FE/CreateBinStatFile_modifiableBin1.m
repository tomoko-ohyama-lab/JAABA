            ...
            if timebins(k,1)>=highrange
                ...
                for l=1:length(allScores.allScores.t0sSeconds{j}) % l=roll ARRAY
                    ...
                    elseif (allScores.allScores.t0sSeconds{j}(l)>=timebins(k,1)&&allScores.allScores.t1sSeconds{j}(l)<=timebins(k,2))
                        output=writeStatFile_roll(1,output,line,j,timebins(k,1),timebins(k,2),allScores.allScores.t0sSeconds{j}(l),allScores.allScores.t1sSeconds{j}(l),tStart,tEnd,selectedCrabspeed);
                    ...
                    elseif ((allScores.allScores.t0sSeconds{j}(l)>=timebins(k,1)&&allScores.allScores.t1sSeconds{j}(l)>timebins(k,2)))
                        output=writeStatFile_roll(1,output,line,j,timebins(k,1),timebins(k,2),allScores.allScores.t0sSeconds{j}(l),timebins(k,2),tStart,tEnd,selectedCrabspeed);
                        line=line+1;
                        output=writeStatFile_roll(1,output,line,j,timebins(k+1,1),timebins(k+1,2),timebins(k+1,1),allScores.allScores.t1sSeconds{j}(l),tStart,tEnd,selectedCrabspeed);
                        line=line+1;
                    else
                        output=writeStatFile_roll(0,output,line,j,timebins(k+1,1),timebins(k+1,2),timebins(k+1,1),allScores.allScores.t1sSeconds{j}(l),tStart,tEnd,selectedCrabspeed);
                        line=line+1;
                    end
                end
            end
