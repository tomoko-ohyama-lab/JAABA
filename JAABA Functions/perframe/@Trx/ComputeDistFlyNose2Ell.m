function d = ComputeDistFlyNose2Ell(obj,n,fly1)
...
% position of nose
xnose = x_mm1 + 2*a_mm1.*cos(theta_mm1);
ynose = y_mm1 + 2*a_mm1.*sin(theta_mm1);
...
for fly2 = 1:nflies,
  ...
  for t = t0:t1,
    i = t - offi;
    j = t - offj;
    d(fly2,i) = ellipsedist_hack(x_mm2(j),y_mm2(j),...
      a_mm2(j),b_mm2(j),theta_mm2(j),...
      xnose(i),ynose(i));
  end
